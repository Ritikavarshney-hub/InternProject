...
IDS_PATH  = "/DATA/bt24eee096/cultural_vlm/results/sample_ids.csv"
META_PATH = "/DATA/bt24eee096/cultural_vlm/results/sample_metadata.csv"
...
inputs_template = tokenizer(
    prompt,
    return_tensors="pt",
    add_special_tokens=False,
)

input_ids_template = inputs_template.input_ids
attention_mask_template = inputs_template.attention_mask

for i, row in enumerate(sample):
    image        = row["image"].convert("RGB")
    pixel_values = preprocess_image(image).to("cuda")
    input_ids    = input_ids_template.to("cuda")
    attention_mask = attention_mask_template.to("cuda")
    with torch.no_grad():
        out = model.generate(
